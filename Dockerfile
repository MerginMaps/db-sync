...
# Create a virtual environment and "activate" it by adding it to PATH
ENV VIRTUAL_ENV=/opt/venv
RUN python3 -m venv $VIRTUAL_ENV
ENV PATH="$VIRTUAL_ENV/bin:$PATH"

# Now pip works normally without flags
RUN pip install --upgrade pip
RUN pip install scikit-build wheel cmake

COPY requirements.txt /tmp/
RUN pip install -r /tmp/requirements.txt

# geodiff is needed with PostgreSQL support - we have to compile it
RUN git clone --branch 2.3.0 https://github.com/merginmaps/geodiff.git
