...
def _print_changes_summary(summary, label=None):
    ...
    logging.debug("Changes:" if label is None else label)
    for item in summary:
        logging.debug("{:20} {:4} {:4} {:4}".format(item['table'], item['insert'], item['update'], item['delete']))
...
def _redownload_project(conn_cfg, mc, work_dir, db_proj_info):
    ...
    logging.debug(f"Downloading version {db_proj_info['version']} of Mergin Maps project {conn_cfg.mergin_project} "
                  f"to {work_dir}")
    try:
    ...
    if from_gpkg:
        ...
        if modified_schema_exists and base_schema_exists:
            ...
            if len(summary_base):
                ...
                logging.debug("Modified schema is not synchronised with source GPKG, please run pull/push commands to fix it")
        ...
        try:
            ...
            logging.debug(f"Cleaning up after a failed DB sync init - dropping schemas {conn_cfg.base} and {conn_cfg.modified}.")
        ...
        if os.path.exists(gpkg_full_path) and base_schema_exists:
            ...
            summary_base = _compare_datasets(conn_cfg.driver, conn_cfg.conn_info, conn_cfg.base,
                                             "sqlite", "", gpkg_full_path, ignored_tables)
            if len(summary_base):
                logging.debug(f"Local project version at {_get_project_version(work_dir)} and base schema at {db_proj_info['version']}")
                _print_changes_summary(summary_base, "Base schema changes:")
                raise DbSyncError("The output GPKG file exists already but is not synchronized with db 'base' schema." 
                                  f"{FORCE_INIT_MESSAGE}")
            elif len(summary_modified):
                logging.debug("The output GPKG file exists already but it is not synchronised with modified schema, "
